        ...
        <th>{{ trans('messages.ACTIONS') }}</th>
            ...
            <tbody>
				...
				{!! Form::model($employee->toArray()+$entity->toArray()+$address->toArray()+$emailaddress->toArray()+$phone->toArray(),['method' => 'PATCH','action' => ['TeammemberController@update', $entity->entityId],'class' => 'form-horizontal']) !!}
					...
					<td id='city'>					{!! Form::hidden('currentCityId', $cityId, ['id' => 'currentCityId']) !!}					{!! Form::select('cityId',$citys,null, ['class' => 'cityDropDown','id'=>'cityValue','placeholder'=>'Select City']) !!}					</td>
					<td>{!! Form::select('locationId',$locations,null, ['id' => 'location','placeholder'=>'Select Location']) !!}</td>
                    <td>
