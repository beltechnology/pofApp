    ...
    <div class="main-table col-md">
        <table class="table table-bordered table-striped table-hover">
            ...
            @foreach($team as $item)
                ...
                <tr>
                    <td>{{ $x }}</td>
                    <td>{{ $item->teamId }}</td><td>{{ $item->teamName }}</td><td>{{ $item->teamName }}</td><td>{{ $item->teamLocation }}</td>
                    <td>
                        <a href="{{ url('/team/' . $item->teamId) }}" class="btn  btn-xs" title="View Team"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"/></a>
